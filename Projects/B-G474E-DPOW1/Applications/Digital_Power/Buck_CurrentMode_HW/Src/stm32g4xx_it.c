...
void FMAC_IRQHandler(void)
{
  ...
  /* Read result from FMAC */
  FmacOutI16 = READ_REG(hfmac.Instance->RDATA);
  
  /* If FMAC output is less than 0 then clamp to 0 */
  if (FmacOutI16 < 0)
    FmacOutI16 = 0;

  /* If FMAC output is greater than DAC MAX then clamp to DAC MAX */
  if (FmacOutI16 > DAC_MAX)
    FmacOutI16 = DAC_MAX;
  
  /* Update sawtooth waveform generator start value */
  LL_DAC_SetWaveSawtoothResetData(DAC4, LL_DAC_CHANNEL_2, FmacOutI16);  
