...
int main(void)
{
  ...
  /* Initialise 3P3Z controller with our 2P2Z coeffs, REF and ADC value */
  CNTRL_3p3zFloatInit(&Demo.CtrlFloat, B0, B1, B2, 0.0,
                      A1, A2, 0.0, K,
                      REF, 0, DAC_MAX );    
...
#ifdef RUN_OPEN_LOOP  
  /* Open loop so run with a small fixed 10% cycle */
  __HAL_HRTIM_SETCOMPARE(&hhrtim1, HRTIM_TIMERINDEX_TIMER_C, HRTIM_COMPAREUNIT_1, Demo.periodBuck * 10 / 100);
#else
