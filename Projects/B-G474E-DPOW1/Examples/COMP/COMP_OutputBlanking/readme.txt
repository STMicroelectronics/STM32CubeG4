...
COMP1 is configured as following:
 ...
 - Output is available on PA6 (pin 24 on connector CN8)
 - Blanking source is connected internally to TIM2 TIM_CHANNEL_3


TIM2 is configured as following:
 - TIM2 frequency is 1kHz.
 - TIM2 TIM_CHANNEL_3  (PB10 - pin 17 on connector CN8) configured in PWM mode.
 - Timer channel intended to be connected to an external device (motor, ...) in user application:
   TIM2 TIM_CHANNEL_1 (PA0 - pin 30 on connector CN8) configured in PWM mode.
 - Both these PWM are running at same frequency of TIM2.

Connection needed:
 - either connect COMP1 input (PA1 - pin 29 on connector CN8) to TIM2 TIM_CHANNEL_1 (PA0 - pin 30 on connector CN8).
 - either connect COMP1 input (PA1 - pin 29 on connector CN8) to a signal generator, at the frequency of TIM2 (to have comparator input signal frequency matching blanking signal frequency). To do this, possibility to use TIM2 TIM_CHANNEL_1 (PA0 - pin 30 on connector CN8) as trigger of signal generator).
...
B-G474E-DPOW1 board's LED is be used to monitor the program execution status:
  - LED4 is toggling every 500ms when test pass successfully.
  - LED4 is "ON" when an error occurred.
...
@par Directory contents 
  ...
  - COMP/COMP_OutputBlanking/Inc/b_g474e_dpow1_conf.h     BSP configuration file
