...
void HRTIM1_TIMD_IRQHandler(void)
{
  ...
  CurrentDuty = LL_HRTIM_TIM_GetCompare3(HRTIM1, LL_HRTIM_TIMER_D);
  if (CurrentDuty < 0xD000)
  {
	CurrentDuty++;
	LL_HRTIM_TIM_SetCompare3(HRTIM1, LL_HRTIM_TIMER_D, CurrentDuty);
  }
  else
  {
	LL_HRTIM_TIM_SetCompare3(HRTIM1, LL_HRTIM_TIMER_D, 0x0220);
  }
