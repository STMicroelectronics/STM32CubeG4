...
- Test description:
      ...
      - The OPAMP1 amplifies output is on PA2 with gain of either 2 or 4.
...
- Connection needed:
      - Connect an oscilloscope to (DAC1/DAC_CHANNEL_1, PA4: pin 41 on connector CN6) 
        to display the sine wave signal generated by the DAC 
        (800 mV amplitude peak to peak). (DAC output = OPAMP input).
      - Connect an oscilloscope to (OPAMP1_OUT, PA2: pin 43 on connector CN6) pin to display waveform  
        (OPAMP output)
      - Press User push-button button to switch between steps.
      - Connect a wire between DAC output (PA4 - pin 41 on connector CN6) to OPAMP non inverting input (PA3 - pin 42 on connector CN6)
...
@par Directory contents 
  ...
  - OPAMP/OPAMP_PGA/Inc/stm32g474e_eval_conf.h     BSP configuration file
